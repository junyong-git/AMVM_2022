...
def append_objs_to_img(cv2_im, inference_size, objs, labels):
    ...
    for obj in objs:
        ...
        if labels.get(obj.id, obj.id) == 'backpack':
            ...
            location_publisher(location) # publish    
        else:
            pass
    return cv2_im

def location_publisher(location_info):
    pub = rospy.Publisher('location_chatter', Int32)
    rospy.init_node('location_publisher')
    while not rospy.is_shutdown():
        rospy.loginfo(location_info)
        pub.publish(String(location_info))
        rospy.sleep(1.0)

# def callback(data):
#     rospy.loginfo(rospy.get_name()+"I heard %s", data.data)

# def loaction_subscriber():
#     rospy.init_node('location_subscriber', anonymous = True) # enable multiple subscribe
